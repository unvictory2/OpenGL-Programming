...
int main()
{
	...
	// --------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "2091184 이승언", NULL, NULL);
	if (window == NULL)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		static float t_total = 0.0f;  // static으로 한번만 초기화, 값 유지됨

		t_total += deltaTime;
		if (t_total > totalMotionTime)
			t_total -= totalMotionTime;
